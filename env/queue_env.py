...
class STargmin(nn.Module):
    ...
    def forward(self, x):
        return F.one_hot(torch.argmin(x), num_classes=x.size()[1]) - self.softmax(
            -x / self.temp).detach() + self.softmax(-x / self.temp)
...
class DiffDiscreteEventSystem(gym.Env):
    def __init__(self, network, mu, h, draw_service, draw_inter_arrivals, init_time=0, batch=1,
        ...
        if queue_event_options is None:
            self.queue_event_options = torch.cat(
                (F.one_hot(torch.arange(0, self.q)), -F.one_hot(torch.arange(0, self.q)))).float().to(self.device)
        else:
    ...
    def reset(self, init_queues=None, time=None, seed=None, options: dict = None):
        ...
        service_times = [[self.draw_service(time) for _ in range(int(queues_sample[q]))] for q in range(self.q) for
                         queues_sample in queues]
    ...
    def step(self, action):
        ...
        for q in range(self.q):
            if num_allocated[q] > 0:
                eff_service_times[q] = torch.stack(service_times[q][:num_allocated[q]])[:, 0, q] / torch.clip(
                    torch.stack(allocated_work[q]), min=self.eps)
        ...
        if True:
            ...
            for q in range(self.q):
                if num_allocated[q] > 0:
                    which_job[q] = int(torch.argmin(
                        torch.stack(service_times[q][:num_allocated[q]]).detach()[:, 0, q] / torch.stack(
                            allocated_work[q])).detach())
        ...
        for q in range(self.q):
            if num_allocated[q] > 0:
                service_times[q][:num_allocated[q]] = list(torch.unbind(
                    torch.stack(service_times[q][:num_allocated[q]]) - event_time * torch.stack(
                        allocated_work[q]).unsqueeze(1).unsqueeze(-1).repeat((1, 1, self.network.shape[-1]))))
        ...
        if True:
            ...
            if torch.sum(delta_arrived) > 0:
                ...
                if torch.sum(delta_arrived) == 1:
                    arrival_times = arrival_times + torch.nan_to_num((new_arrival_times) * delta_arrived,
                                                                     nan=self.inv_eps)
            ...
            if torch.sum(delta_left) > 0:
                # remove a served job
                service_times[which_queue][which_job[which_queue]] = service_times[which_queue][
                    which_job[which_queue]].detach()
