...
class BankSimulation(environment.Environment[EnvState, EnvParames]):
    ...
    def step_env(
        ...
        # Precompute a flag for the event case (0: arriving, 1: processing, 2: equal time)
        event_case = jnp.sign(expected_next_arriving_time - expected_next_processing_time) + 1

        # Update values based on the event case
        def update_customer_arrival():
            customer_in_the_queue = state.customers_in_the_queue + 1
            clock_time = state.last_customer_enter_time + state.customers_arriving_time
            customers_arriving_time = jax.random.poisson(key, lam=12.5)
            return (
                customer_in_the_queue,
                clock_time,
                customers_arriving_time,
                clock_time,
                state.last_clerk_processing_time,
            )

        def update_clerk_process():
            customer_in_the_queue = state.customers_in_the_queue - 1
            clock_time = state.last_clerk_processing_time + params.clerk_processing_time
            return (
                customer_in_the_queue,
                clock_time,
                state.customers_arriving_time,
                state.last_customer_enter_time,
                clock_time,
            )

        def update_equal_time():
            clock_time = expected_next_processing_time
            customers_arriving_time = jax.random.poisson(key, lam=12.5)
            return (
                state.customers_in_the_queue,
                clock_time,
                customers_arriving_time,
                clock_time,
                clock_time,
        ...
        # Select the appropriate update using a numerical map
        updates = jnp.array([
            update_customer_arrival(),
            update_clerk_process(),
            update_equal_time()
        ])

        selected_update = updates[event_case.astype(int)]
        (
            customers_in_the_queue,
            clock_time,
            customers_arriving_time,
            last_customer_enter_time,
        ...
        for t_idx in range(len(result['time'])):
            ...
            if t_idx > 0:
                ...
                if curr_num > prev_num:
                    ...
                    event_label = "Customer Arrive and Customer Served"
            ...
            # Save frame
            # ax.set_title(f"Rollout for Worker: {key} at time: {real_time_str[-1]} | Events: {event_label}",
